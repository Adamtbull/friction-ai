export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/store/specials" && request.method === "POST") {
      ...
      // Fallback specials to use if anything fails
      ...
      try {
        ...
        // Log the error but continue with fallback specials
        console.log("Store specials search failed, using fallback:", searchErr.message);
      }

      // ALWAYS return 200 with specials (real or fallback)
...
async function extractRosterFromImage(imageBase64, options, env) {
  ...
  var prompt = [
    ...
    "",
    "NURSING/HEALTHCARE SHIFT CODES (VERY IMPORTANT):",
    "- A or AM = Morning shift: 07:00 to 15:30",
    "- P or PM = Afternoon shift: 13:30 to 22:00",
    "- N or ND or Night = Night shift: 21:30 to 07:30",
    "- If specific times are shown next to the letter (e.g. 'A 0700-1430' or 'P 1430-2200'), use those exact times",
    "- The letter code tells you the shift TYPE, but always check for specific times that override the default",
    "",
    "IMPORTANT RULES:",
    ...
    "- Current month context: look at the month name/number shown in the roster",
    "- Convert all dates to YYYY-MM-DD format",
    "- Convert all times to 24-hour HH:MM format",
    "- If you see day names (Mon, Tue, etc), calculate the actual date based on the month shown",
    "- If location/ward/department is shown, include it",
    "- Include break times in notes if shown",
    "- Night shifts that span midnight: use the START date, with times 21:30 to 07:30",
    "- RDO, OFF, LEAVE, ADO, AL, SL days should be included with notes indicating the type",
    "- X or blank days = no shift, skip these",
